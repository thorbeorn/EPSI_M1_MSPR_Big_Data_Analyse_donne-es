"""
=============================================================
  PRÉDICTION ÉLECTORALE — MLP (RÉGRESSION)
...
def train_logistic_model(df_indicateurs, df_resultats, annee_cible=2024):
    """
    Entraîne un MLPRegressor pour chaque famille politique.
    ...
    for col, label in zip(familles_cibles, familles_labels):
        ...
        logger.info(f"  MLP {label}: MAE={metrics[label]['MAE']}, R2={metrics[label]['R2']}")
    ...
    logger.info(f"MLP: résultat national 2024 → {national} | vainqueur={vainqueur_national}")
