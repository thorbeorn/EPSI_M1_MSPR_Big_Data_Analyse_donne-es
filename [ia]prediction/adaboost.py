...
def train_logistic_model(df_indicateurs, df_resultats):
    """
    Entraîne un modèle AdaBoost (Adaptive Boosting).
    Combine 100 arbres de décision faibles qui corrigent les erreurs
    des précédents à chaque itération.
    Retourne : (accuracy, report, resultats_2024, mae, r2)
    """

    logger.info("AdaBoost: début de l'entraînement")

    if 'code_departement' in df_resultats.columns:
        df_resultats = df_resultats.rename(columns={'code_departement': 'Code_departement'})
        logger.debug("AdaBoost: renommage de la colonne code_departement")
    ...
    logger.debug(f"AdaBoost: train/test split, train={X_train.shape}, test={X_test.shape}")
    ...
    logger.info("AdaBoost: entraînement terminé")
    ...
    logger.info(f"AdaBoost: évaluation terminée - accuracy={accuracy:.4f}, mae={mae:.4f}, r2={r2:.4f}")

    model.fit(X_scaled, y_encoded)
    logger.info("AdaBoost: ré-entraînement sur l'ensemble des données terminé")
