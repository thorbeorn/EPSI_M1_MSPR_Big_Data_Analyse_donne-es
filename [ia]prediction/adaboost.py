"""
=============================================================
  PRÉDICTION ÉLECTORALE — ADABOOST (RÉGRESSION)
...
def train_logistic_model(df_indicateurs, df_resultats, annee_cible=2024):
    """
    Entraîne un AdaBoostRegressor pour chaque famille politique.
    ...
    for col, label in zip(familles_cibles, familles_labels):
        ...
        logger.info(f"  AdaBoost {label}: MAE={metrics[label]['MAE']}, R2={metrics[label]['R2']}")
    ...
    for col, label in zip(familles_cibles, familles_labels):
        ...
        ada.fit(X_full, y_full)
        models_final[label] = ada
    ...
    logger.info(f"AdaBoost: résultat national 2024 → {national} | vainqueur={vainqueur_national}")

    return {
        "modele": "AdaBoostRegressor",
